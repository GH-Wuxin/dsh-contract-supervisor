// S5.2 — RunSpec v1 parser and validator.
//
// RunSpec is the developer-only JSON input contract. It carries trusted human
// inputs ONLY: repoRoot, the authentic FrozenContract creation input, and the
// authentic FrozenSlice creation input (excluding the derived contractHash,
// which the host injects from the created FrozenContract).
//
// The RunSpec MUST NOT authoritatively provide identity, authority, FS, or
// model-configuration fields. Such fields are rejected (not ignored) so the
// trusted input seam cannot be confused into widening authority or pinning a
// different model.
import type { ContractInput, SliceInput } from '../domain/index.js';
import {
  DRIVER_ERROR_CODES,
  DriverError,
  REJECTED_RUNSPEC_SLICE_KEYS,
  ...
  for (const key of rejected) {
    if (Object.prototype.hasOwnProperty.call(obj, key)) {
      throw new DriverError(
        DRIVER_ERROR_CODES.RUNSPEC_REJECTED_FIELD,
        `RunSpec ${where} must not supply derived/authority field '${key}'; it is host-derived and cannot be supplied by trusted input`,
      );
    }
  }
}
 ...
 */
export function parseRunSpec(raw: unknown): ParsedRunSpec {
  if (!isPlainObject(raw)) {
    throw new DriverError(
      DRIVER_ERROR_CODES.RUNSPEC_INVALID,
      'RunSpec must be a plain JSON object',
    );
  }
  ...
  if (version !== RUNSPEC_VERSION) {
    throw new DriverError(
      DRIVER_ERROR_CODES.RUNSPEC_VERSION_UNSUPPORTED,
      `RunSpec version must be ${RUNSPEC_VERSION}; received ${JSON.stringify(version)}`,
    );
  }
  ...
  if (typeof repoRoot !== 'string' || repoRoot.length === 0) {
    throw new DriverError(
      DRIVER_ERROR_CODES.RUNSPEC_INVALID,
      "RunSpec field 'repoRoot' must be a non-empty string",
    );
  }
  if (repoRoot.includes('\0')) {
    throw new DriverError(
      DRIVER_ERROR_CODES.RUNSPEC_INVALID,
      "RunSpec field 'repoRoot' must not contain NUL bytes",
    );
  }
  ...
  if (!isPlainObject(contractRaw)) {
    throw new DriverError(
      DRIVER_ERROR_CODES.RUNSPEC_INVALID,
      "RunSpec field 'contract' must be a plain object",
    );
  }
  ...
  if (Object.prototype.hasOwnProperty.call(contractRaw, 'contractHash')) {
    throw new DriverError(
      DRIVER_ERROR_CODES.RUNSPEC_REJECTED_FIELD,
      "RunSpec contract must not supply 'contractHash'; it is host-derived from authentic FrozenContract creation",
    );
  }
  ...
  if (!isPlainObject(sliceRaw)) {
    throw new DriverError(
      DRIVER_ERROR_CODES.RUNSPEC_INVALID,
      "RunSpec field 'slice' must be a plain object",
    );
  }
  ...
  const contractInput = contractRaw as unknown as ContractInput;
  ...
  const sliceInputBase = sliceRaw as unknown as Omit<SliceInput, 'contractHash'>;
