...
import type { SupervisorRuntimeState } from '../state/index.js';
import type { WorkerAttemptResult, WorkerLifecycleCoordinator } from '../worker/index.js';
...
import {
  DRIVER_COMMANDER_MODEL,
  DRIVER_COMMANDER_PROVIDER,
  DRIVER_ERROR_CODES,
  DriverError,
  EXIT_CODE_COMMANDER_FAILURE,
  EXIT_CODE_PRE_COMMANDER_FAILURE,
  EXIT_CODE_SUCCESS,
  EXIT_CODE_WORKER_FAILURE,
  type DriverResult,
  type WorkerOutcome,
} from './types.js';
...
function failureResult(
  ...
  code: string,
  message: string,
  workerOutcome: WorkerOutcome | null = null,
  workerPhase: string | null = null,
  workerSettled = false,
): DriverResult {
  return {
    ok: false,
...
function asDriverError(error: unknown): DriverError {
...
function buildFlashWorkerConfig() {
  return Object.freeze({
    role: 'implementation_worker' as const,
    provider: 'deepseek-ai' as const,
 ...
 */
export async function runContractSupervisorDriver(
  ctx: DriverContext,
  rawRunSpec: unknown,
  trace?: DriverTrace,
  ...
  let proHandle: AgentHandle | null = null;
  ...
  try {
    ...
    try {
      ...
      return failureResult(
        stage,
        EXIT_CODE_PRE_COMMANDER_FAILURE,
        de.code === DRIVER_ERROR_CODES.DRIVER_INTERNAL
          ? DRIVER_ERROR_CODES.CONTRACT_CREATION_FAILED
          : de.code,
        `FrozenContract creation failed: ${de.message}`,
      );
    }
    ...
    try {
      ...
      return failureResult(
        ...
        de.code === DRIVER_ERROR_CODES.DRIVER_INTERNAL
          ...
          : de.code,
        `FrozenSlice creation failed: ${de.message}`,
      );
    }
    ...
    try {
      ...
      return failureResult(
        stage,
        EXIT_CODE_PRE_COMMANDER_FAILURE,
        DRIVER_ERROR_CODES.ADMISSION_FAILED,
        `Slice admission failed: ${de.message}`,
      );
    }
    ...
    const commanderPrompt = buildCommanderPrompt(
      contract,
      slice,
      effectiveWorkerToolNames,
    );

    const proSetup: CreateAgentOptions['setup'] = (agentCtx) => {
    ...
    try {
      ...
      return failureResult(
        stage,
        EXIT_CODE_COMMANDER_FAILURE,
        DRIVER_ERROR_CODES.COMMANDER_SPAWN_FAILED,
        `Pro commander spawn failed: ${de.message}`,
      );
    }
    ...
    try {
      ...
      return failureResult(
        stage,
        EXIT_CODE_COMMANDER_FAILURE,
        DRIVER_ERROR_CODES.COMMANDER_RUNTIME_FAILED,
        `Pro commander runtime failed: ${de.message}`,
      );
    }
    ...
    let commanderTurn: number;
    try {
      const extracted = extractCommanderInstruction(
        commanderEvents,
        String(proAgent.session.id),
      );
    ...
    const port = trace === undefined
      ? rawPort
      : {
          spawn: async (request: Parameters<typeof rawPort.spawn>[0]) => {
    ...
    const coordinator: WorkerLifecycleCoordinator =
      service.createWorkerLifecycleCoordinator(
        port,
        flashConfig,
        sessions,
        {
    ...
    let attemptResult: WorkerAttemptResult;
    try {
      ...
      return failureResult(
        stage,
        EXIT_CODE_WORKER_FAILURE,
        DRIVER_ERROR_CODES.WORKER_ATTEMPT_FAILED,
        `Worker attempt threw: ${de.message}`,
      );
    }
    ...
    return failureResult(
      stage,
      EXIT_CODE_WORKER_FAILURE,
      err ? (err as { code?: string }).code ?? DRIVER_ERROR_CODES.WORKER_ATTEMPT_FAILED : DRIVER_ERROR_CODES.WORKER_ATTEMPT_FAILED,
      err ? err.message : `Worker attempt settled with outcome ${String(workerOutcome)} (settled=${String(workerSettled)})`,
      workerOutcome,
      workerPhase,
      workerSettled,
    );
