...
import type { AttemptState } from '../state/index.js';
...
import type { PreparedSliceFsAuthority } from '../fs/authority.js';
import {
...
import {
  WORKER_FS_BINDING,
  createWorkerFsBindingRequest,
  type FrozenWorkerConfig,
  type WorkerAttemptInput,
...
export function createWorkerLifecycleCoordinator(
  ...
  sliceFsSessions: SliceFsSessionRegistry = createSliceFsSessionRegistry(),
  workerFsConfig?: WorkerFsConfig,
): WorkerLifecycleCoordinator {
  return new WorkerLifecycleCoordinator(
    port,
    config,
    sliceFsSessions,
    workerFsConfig,
...
interface FrozenWorkerFsCatalog {
  readonly repoRoot: string;
  readonly slices: ReadonlyMap<string, FrozenSlice>;
}

function freezeWorkerFsConfig(
  config: WorkerFsConfig | undefined,
): FrozenWorkerFsCatalog | null {
  if (config === undefined) {
    return null;
  }
  ...
  if (
    ...
    throw new WorkerError(
      ...
      'Worker filesystem config must provide a repoRoot string and a frozen Slice array',
    );
  }

  const slices = new Map<string, FrozenSlice>();
  for (const [index, slice] of config.slices.entries()) {
    if (!(slice instanceof FrozenSlice)) {
      throw new WorkerError(
        WORKER_ERROR_CODES.WORKER_CONFIGURATION_INVALID,
        `Worker filesystem config slices[${index}] is not an authentic FrozenSlice`,
      );
    }
  ...
  try {
    ...
    throw new WorkerError(
      WORKER_ERROR_CODES.WORKER_CONFIGURATION_INVALID,
      `Worker filesystem config repository root is invalid: ${
...
function invalidatedFsResult(violation: FsError): WorkerResult {
  return {
    outcome: 'INVALIDATED',
...
export class WorkerLifecycleCoordinator {
  ...
  private readonly fsCatalog: FrozenWorkerFsCatalog | null;
  ...
  private deriveAttemptFsAuthority(
    ...
    if (frozenSlice === undefined) {
      throw new WorkerError(
        WORKER_ERROR_CODES.ACTIVE_SLICE_AUTHORITY_NOT_RECOVERABLE,
        `Active Slice '${attempt.sliceHash}' has no frozen Slice entry in the Supervisor-owned S5 filesystem config; filesystem authority is not recoverable`,
      );
    }
  ...
  private deriveAttemptEffectiveToolAllowlist(
    ...
    if (frozenSlice === undefined) {
      throw new WorkerError(
        WORKER_ERROR_CODES.ACTIVE_SLICE_AUTHORITY_NOT_RECOVERABLE,
        `Active Slice '${attempt.sliceHash}' has no frozen Slice entry in the Supervisor-owned S5 filesystem config; filesystem tool policy is not recoverable`,
      );
    }
    ...
    for (const tool of requested) {
      if (!this.config.toolAllowlist.includes(tool)) {
        throw new WorkerError(
          WORKER_ERROR_CODES.ACTIVE_SLICE_TOOL_POLICY_NOT_RECOVERABLE,
          `Active Slice '${attempt.sliceHash}' requests worker tool '${tool}' outside the FrozenWorkerConfig upper bound [${this.config.toolAllowlist.join(', ')}]; supported universe is [${SLICE_FS_TOOL_NAMES.join(', ')}]`,
        );
      }
    }
      ...
      if (fsAuthority !== null && !this.sliceFsSessions.hasLiveSession(run.sessionId)) {
        try {
          ...
          if (disposeFailure !== undefined) {
            ...
            return {
              ...
              error: asWorkerError(
                ...
                `Worker dispose failed while recovering from an S5 session binding failure for attempt '${input.attemptId}' (binding: ${executionErrorMessage(error)}; dispose: ${executionErrorMessage(disposeFailure)})`,
                disposeFailure,
              ),
              settled: false,
            };
          ...
          return {
            ...
            error: asWorkerError(
              WORKER_ERROR_CODES.WORKER_SPAWN_FAILED,
              `Worker filesystem session binding failed for attempt '${input.attemptId}': ${executionErrorMessage(error)}`,
              error,
            ),
            settled: true,
          };
