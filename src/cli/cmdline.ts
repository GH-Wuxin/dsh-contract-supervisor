 ...
 */
function buildProgram(ctx: CmdlineContext): Command {
  ...
  program
    ...
    .action(async (options: { spec: string }) => {
      ...
      const fail = (message: string): void => {
        const stderr = (process as { stderr: { write(chunk: string): unknown } }).stderr;
      ...
      try {
        ...
        const stdout = (process as { stdout: { write(chunk: string): unknown } }).stdout;
        stdout.write(`${JSON.stringify(result)}\n`);
        if (typeof exit === 'function') exit(result.exitCode);
 ...
 * service unchanged.
 */
export function maybeRunContractSupervisorCmdline(ctx: CmdlineContext): boolean {
