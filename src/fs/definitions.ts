 ...
 */
export type SliceFsSessionResolver = (
  execution: ToolExecution,
) => string | null | undefined;

export function resolveSliceFsSessionFromExecution(
  execution: ToolExecution,
): string | null {
...
function requireSession(execution: ToolExecution, resolver: SliceFsSessionResolver): string {
  ...
  if (sessionId === undefined || sessionId === null || sessionId.length === 0) {
    throw new FsError(
      ...
      'No trusted worker session identity is available for this audited filesystem tool',
    );
  }
  return sessionId;
}

function textRender(value: unknown): Array<{ type: 'text'; text: string }> {
 ...
 */
export function createSliceFsToolDefinitions(
  runtime: SliceFsRuntime,
  resolveSessionId: SliceFsSessionResolver = resolveSliceFsSessionFromExecution,
): Readonly<Record<SliceFsToolName, ToolDefinition>> {
  const definitions = {
    slice_read: defineTool({
      name: 'slice_read',
      description:
        'Read one text file only when its canonical repo-relative path is inside the current Slice allowedReads authority.',
      parameters: {
    ...
    slice_search: defineTool({
      name: 'slice_search',
      description:
        'Literal-text search over files inside the current Slice allowedReads authority only.',
      parameters: {
    ...
    slice_write: defineTool({
      name: 'slice_write',
      description:
        'Write one exact target that is present in the current Slice allowedWrites authority.',
      parameters: {
    ...
    slice_edit: defineTool({
      name: 'slice_edit',
      description:
        'Replace exactly one occurrence of oldText with newText in one exact allowedWrites target.',
      parameters: {
        path: { type: 'string', required: true, description: 'Exact repo-relative write target' },
