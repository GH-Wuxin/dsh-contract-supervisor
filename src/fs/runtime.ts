...
function invalidArgs(message: string): FsError {
  return new FsError(FS_ERROR_CODES.FS_INVALID_ARGUMENT, message);
}

function readScopeViolation(path: string): FsError {
  return new FsError(
    FS_ERROR_CODES.SLICE_READ_SCOPE_VIOLATION,
    `Path '${path}' is outside this Slice's allowedReads authority`,
  );
}

function writeScopeViolation(path: string, detail?: string): FsError {
  return new FsError(
    FS_ERROR_CODES.SLICE_WRITE_SCOPE_VIOLATION,
    `Path '${path}' is outside this Slice's allowedWrites authority${detail ?? ''}`,
  );
}

function operationFailure(message: string): FsError {
...
function editMismatch(message: string): FsError {
...
function parseExactArgs(
  ...
  for (const key of Object.keys(args)) {
    if (!allowed.has(key)) {
      throw invalidArgs(
        `${toolName} does not accept field '${key}'; filesystem authority can never be supplied by tool arguments`,
      );
    }
  }
...
interface VerifiedExistingFile {
  ...
  readonly before: Awaited<ReturnType<typeof import('node:fs/promises').lstat>>;
  readonly after: Awaited<ReturnType<typeof import('node:fs/promises').lstat>>;
}

async function openVerifiedExistingFile(
  absolute: string,
  before: NonNullable<Awaited<ReturnType<typeof import('node:fs/promises').lstat>>>,
): Promise<VerifiedExistingFile> {
  if (!before.isFile()) {
    throw identityUnsafe(
      `Target '${absolute}' is not a regular file`,
    );
  }
  if (!Number.isInteger(before.nlink) || before.nlink !== 1) {
    throw identityUnsafe(
      `Target '${absolute}' has hardlink count ${String(before.nlink)}; exactly 1 is required`,
    );
  }
  ...
  try {
    ...
    throw operationFailure(
      `Cannot open target '${absolute}': ${error instanceof Error ? error.message : String(error)}`,
    );
  }

  let after: Awaited<ReturnType<typeof import('node:fs/promises').lstat>>;
  try {
    after = await handle.stat();
  } catch (error) {
    await closeQuietly(handle);
    throw identityUnsafe(
      `Cannot obtain open-file identity for '${absolute}': ${
  ...
  if (!assertSameFileIdentity(before, after)) {
    await closeQuietly(handle);
    throw symlinkBlock(
      `Target '${absolute}' changed identity between lstat and open; symlink/alias mutation is blocked`,
    );
  }
  ...
  if (!Number.isInteger(after.nlink) || after.nlink !== 1) {
    await closeQuietly(handle);
    throw identityUnsafe(
      `Open target '${absolute}' has hardlink count ${String(after.nlink)}; exactly 1 is required`,
    );
  }
...
async function mutateThroughHandle(
  ...
  try {
    ...
    throw operationFailure(
      `Cannot write target '${absolute}': ${error instanceof Error ? error.message : String(error)}`,
    );
  }
...
async function createNewFile(
  ...
  try {
    ...
    if (isErrno(error, 'EEXIST')) {
      throw operationFailure(
        `Target '${relativePath}' appeared between authorization and create; no mutation was attempted`,
      );
    }
    throw operationFailure(
      `Cannot create target '${relativePath}': ${error instanceof Error ? error.message : String(error)}`,
    );
  }
  ...
  try {
    ...
    try {
      ...
      throw identityUnsafe(
        `Cannot obtain identity metadata for newly created '${relativePath}': ${
          error instanceof Error ? error.message : String(error)
        }`,
      );
    }
    ...
    if (!Number.isInteger(after.nlink) || after.nlink !== 1) {
      throw identityUnsafe(
        `Newly created target '${relativePath}' has hardlink count ${String(after.nlink)}; exactly 1 is required`,
      );
    }
    ...
    throw operationFailure(
      `Cannot write newly created target '${relativePath}': ${
        error instanceof Error ? error.message : String(error)
      }`,
    );
  }
...
export class SliceFsRuntime {
  ...
  private readonly lstatImpl: PathLstat;

  constructor(
    sessions: SliceFsSessionRegistry = createSliceFsSessionRegistry(),
    options: SliceFsRuntimeOptions = {},
  ) {
    if (
      options.lstat !== undefined &&
  ...
  private async audit<T>(
    sessionId: string,
    operation: (binding: SliceFsSessionBinding) => Promise<T>,
  ): Promise<T> {
  ...
  async read(sessionId: string, args: unknown): Promise<SliceFsReadResult> {
    return this.audit(sessionId, async (binding) => {
      const parsed = parseExactArgs('slice_read', args, ['path']) as unknown as SliceReadRequest;
      const path = requireString(parsed.path, 'path');
      const authority = binding.authority;
      const resolved = resolveReadPath(authority, path);

      const inspection = await inspectPathNoSymlinksWithOps(
        authority.repoRoot,
        resolved.absolute,
        'read',
        this.lstatImpl,
      ...
      try {
        ...
        throw operationFailure(
          `Cannot open read target '${resolved.relative}': ${
            error instanceof Error ? error.message : String(error)
          }`,
        );
      }
      ...
      try {
        ...
        if (!assertSameFileIdentity(before, after)) {
          throw symlinkBlock(
            `Read target '${resolved.relative}' changed identity between lstat and open`,
          );
        }
        ...
        throw operationFailure(
          `Cannot read target '${resolved.relative}': ${
            error instanceof Error ? error.message : String(error)
          }`,
        );
      } finally {
  ...
  async write(sessionId: string, args: unknown): Promise<SliceFsWriteResult> {
    return this.audit(sessionId, async (binding) => {
      const parsed = parseExactArgs('slice_write', args, ['path', 'content']) as unknown as SliceWriteRequest;
      const path = requireString(parsed.path, 'path');
      ...
      if (inspection.final === 'missing' || inspection.finalStats === null) {
        if (!hasWriteOperation(authority, resolved.relative, 'create')) {
          throw writeScopeViolation(
            resolved.relative,
            ': target does not exist and only update authority is held',
          );
        }
      ...
      if (!hasWriteOperation(authority, resolved.relative, 'update')) {
        throw writeScopeViolation(
          resolved.relative,
          ': target exists and only create authority is held',
        );
      }
  ...
  async edit(sessionId: string, args: unknown): Promise<SliceFsEditResult> {
    return this.audit(sessionId, async (binding) => {
      const parsed = parseExactArgs(
        'slice_edit',
        args,
        ['path', 'oldText', 'newText'],
      ) as unknown as SliceEditRequest;
      const path = requireString(parsed.path, 'path');
      ...
      if (!hasWriteOperation(authority, resolved.relative, 'update')) {
        throw writeScopeViolation(
          resolved.relative,
          ': slice_edit requires update authority for an existing target',
        );
      }
      ...
      try {
        ...
        try {
          ...
          throw operationFailure(
            `Cannot read edit target '${resolved.relative}': ${
              error instanceof Error ? error.message : String(error)
            }`,
          );
        }
        ...
        if (occurrences !== 1) {
          throw editMismatch(
            `Edit target '${resolved.relative}' contains ${occurrences} occurrence(s) of oldText; exactly 1 is required`,
          );
        }
  ...
  async search(sessionId: string, args: unknown): Promise<SliceFsSearchResult> {
    return this.audit(sessionId, async (binding) => {
      if (!isRecord(args)) {
      ...
      for (const key of Object.keys(args)) {
        if (key !== 'path' && key !== 'pattern') {
          throw invalidArgs(
            `slice_search does not accept field '${key}'; search scope can never be widened by tool arguments`,
          );
        }
      }
      ...
      const matches: SliceFsSearchMatch[] = [];
      let filesSearched = 0;
      ...
      for (const file of searchFiles) {
        ...
        try {
          ...
          throw operationFailure(
            `Cannot search authorized file '${file.relative}': ${
              error instanceof Error ? error.message : String(error)
            }`,
          );
        }
  ...
  private async collectSearchFiles(
    binding: SliceFsSessionBinding,
    rawPath: unknown,
  ): Promise<Array<{ readonly absolute: string; readonly relative: string }>> {
    const authority = binding.authority;
    const files = new Map<string, { readonly absolute: string; readonly relative: string }>();
    ...
    for (const rule of authority.readRules) {
      ...
      if (rule.recursive) {
        if (!inspection.finalStats.isDirectory()) {
          throw operationFailure(
            `Recursive allowedReads entry '${rule.raw}' is not a directory`,
          );
        }
  ...
  private async walkAuthorizedDirectory(
    authority: PreparedSliceFsAuthority,
    absoluteRoot: string,
    files: Map<string, { readonly absolute: string; readonly relative: string }>,
  ): Promise<void> {
    ...
    while (pending.length > 0) {
      ...
      try {
        entries = await readdir(directory, { withFileTypes: true }) as unknown as typeof entries;
      } catch (error) {
        if (isErrno(error, 'ENOENT')) {
          continue;
        }
        throw operationFailure(
          `Cannot enumerate authorized directory '${portableRelative(relative(authority.repoRoot, directory))}': ${
            error instanceof Error ? error.message : String(error)
          }`,
        );
      }
...
export function createSliceFsRuntime(
  sessions: SliceFsSessionRegistry = createSliceFsSessionRegistry(),
  options: SliceFsRuntimeOptions = {},
): SliceFsRuntime {
...
export async function sliceRead(
  ...
  sessionId: string,
  args: unknown,
): Promise<SliceFsReadResult> {
  return new SliceFsRuntime(sessions).read(sessionId, args);
}

export async function sliceWrite(
  sessions: SliceFsSessionRegistry,
  sessionId: string,
  args: unknown,
): Promise<SliceFsWriteResult> {
...
export async function sliceEdit(
  sessions: SliceFsSessionRegistry,
  sessionId: string,
  args: unknown,
): Promise<SliceFsEditResult> {
  return new SliceFsRuntime(sessions).edit(sessionId, args);
}

export async function sliceSearch(
  sessions: SliceFsSessionRegistry,
  sessionId: string,
  args: unknown,
): Promise<SliceFsSearchResult> {
