...
function requireString(value, field) {
    if (typeof value !== 'string') {
        throw new DomainError(ERROR_CODES.INVALID_CONTRACT, `Contract field '${field}' must be a string`);
    }
    return value;
}
function requireStringArray(value, field) {
    if (!Array.isArray(value) || !value.every((item) => typeof item === 'string')) {
        throw new DomainError(ERROR_CODES.INVALID_CONTRACT, `Contract field '${field}' must be an array of strings`);
    }
    return [...value];
}
function requireWriteAuthority(value) {
    if (!Array.isArray(value)) {
        throw new DomainError(ERROR_CODES.INVALID_CONTRACT, "Contract field 'writeAuthority' must be an array");
    }
    return value.map((item) => {
        if (!isRecord(item) || typeof item.path !== 'string') {
            throw new DomainError(ERROR_CODES.INVALID_CONTRACT, "Contract writeAuthority entry must be an object with a string 'path'");
        }
        if (item.operation !== 'create' && item.operation !== 'update') {
            throw new DomainError(ERROR_CODES.INVALID_CONTRACT, "Contract writeAuthority operation must be 'create' or 'update'");
        }
        return {
            path: item.path,
...
function requireVerifierCatalog(value) {
    if (!Array.isArray(value)) {
        throw new DomainError(ERROR_CODES.INVALID_CONTRACT, "Contract field 'verifierCatalog' must be an array");
    }
    return value.map((item) => {
        if (!isRecord(item) || typeof item.verifierId !== 'string') {
            throw new DomainError(ERROR_CODES.INVALID_CONTRACT, "Contract verifierCatalog entry must have a string 'verifierId'");
        }
...
export function normalizeContractInput(input) {
    if (!isRecord(input)) {
        throw new DomainError(ERROR_CODES.INVALID_CONTRACT, 'Contract input must be a plain object');
    }
    const parentContractHash = input.parentContractHash === undefined || input.parentContractHash === null
        ? null
