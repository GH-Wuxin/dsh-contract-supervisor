...
import { canonicalizeRepositoryRoot, isInsideRoot, portableRelative, resolveRequestPath, validateRequestPath, } from './path.js';
const AUTHORITY_BRAND = Symbol('dsh-contract-supervisor.sliceFsAuthority');
const preparedAuthorityInternals = new WeakMap();
function authorityInvalid(message) {
    return new FsError(FS_ERROR_CODES.FS_AUTHORITY_CONFIG_INVALID, message);
}
function validateSessionIdentity(value, field) {
 ...
 */
function normalizeAuthorityBase(repoRoot, input, field) {
    ...
    try {
        ...
        throw authorityInvalid(`${field} is not a safe repo-relative path: ${error instanceof Error ? error.message : String(error)}`);
    }
    const segments = portable.split('/');
    if (segments.some((segment) => segment === '' || segment === '.' || segment === '..')) {
        throw authorityInvalid(`${field} must be normalized (no empty, '.' or '..' segments), received '${input}'`);
    }
...
function getPreparedAuthorityInternals(authority) {
...
export function isPreparedSliceFsAuthority(value) {
    return (typeof value === 'object' &&
        value !== null &&
        value[AUTHORITY_BRAND] === true &&
        preparedAuthorityInternals.has(value));
}
/**
 ...
 */
export function createSliceFsAuthority(input) {
    ...
    const readRules = input.allowedReads.map((entry, index) => {
        ...
        if (/[*?[\]{}]/.test(portable)) {
            throw authorityInvalid(`${field} uses unsupported wildcard syntax; only exact paths and 'dir/**' are supported`);
        }
    ...
    const writeRules = input.allowedWrites.map((rule, index) => {
    ...
    const exactReadPaths = new Set(readRules.filter((rule) => !rule.recursive).map((rule) => rule.key));
    const prepared = {
        repoRoot,
        sliceId,
        allowedReads: Object.freeze([...input.allowedReads]),
        allowedWrites: Object.freeze(input.allowedWrites.map((rule) => Object.freeze({ ...rule }))),
        ...
        recursiveReadPrefixes: Object.freeze(readRules.filter((rule) => rule.recursive).map((rule) => rule.key)),
    };
...
export function isReadPathAllowed(authority, relativePath) {
    const key = pathKey(relativePath);
    const internals = getPreparedAuthorityInternals(authority);
    if (internals.exactReadPaths.has(key)) {
        return true;
    }
    return authority.recursiveReadPrefixes.some((prefix) => key.startsWith(`${prefix}/`));
}
/**
 ...
 */
export function isReadSearchRootAllowed(authority, relativePath) {
    const key = pathKey(relativePath);
    const internals = getPreparedAuthorityInternals(authority);
    if (internals.exactReadPaths.has(key)) {
        return true;
    }
    return authority.recursiveReadPrefixes.some((prefix) => key === prefix || key.startsWith(`${prefix}/`));
}
export function hasWritePath(authority, relativePath) {
    return getPreparedAuthorityInternals(authority).writeOperations.has(pathKey(relativePath));
}
export function hasWriteOperation(authority, relativePath, operation) {
    return (getPreparedAuthorityInternals(authority)
        .writeOperations.get(pathKey(relativePath))
        ?.has(operation) ?? false);
}
export function resolveAuthorityPath(authority, rawPath) {
