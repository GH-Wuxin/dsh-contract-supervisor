...
import { isUniqueSubsetOfSliceFsTools, SLICE_FS_TOOL_NAMES, } from '../fs/toolNames.js';
import { WORKER_MODEL, WORKER_PRESENTATION, WORKER_ROLE, } from './types.js';
/**
 ...
 */
export const FORBIDDEN_WORKER_TOOLS = Object.freeze([]);
export const SUPPORTED_WORKER_TOOLS = SLICE_FS_TOOL_NAMES;
function invalid(message) {
    return new WorkerError(WORKER_ERROR_CODES.WORKER_CONFIGURATION_INVALID, message);
}
export function assertValidWorkerConfig(config) {
    if (config.role !== WORKER_ROLE) {
        throw invalid(`Worker role must be '${WORKER_ROLE}', received '${String(config.role)}'`);
    }
    if (config.provider !== 'deepseek-ai') {
        throw invalid(`Worker provider must be 'deepseek-ai', received '${String(config.provider)}'`);
    }
    if (config.model !== WORKER_MODEL) {
        throw invalid(`Worker model must be '${WORKER_MODEL}', received '${String(config.model)}'`);
    }
    if (config.presentation !== WORKER_PRESENTATION) {
        throw invalid(`Worker presentation must be '${WORKER_PRESENTATION}', received '${String(config.presentation)}'`);
    }
    if (config.oneShot !== true) {
        throw invalid('Worker must be configured as oneShot=true');
    }
    if (config.maxDepth !== 1) {
        throw invalid(`Worker maxDepth must be exactly 1, received '${String(config.maxDepth)}'`);
    }
    ...
    if (!isUniqueSubsetOfSliceFsTools(config.toolAllowlist)) {
        ...
        throw invalid(`Worker toolAllowlist must be a duplicate-free subset of [${SLICE_FS_TOOL_NAMES.join(', ')}], received [${rendered}]`);
    }
}
export function freezeWorkerConfig(config) {
