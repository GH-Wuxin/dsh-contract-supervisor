...
import { COMMANDER_OUTPUT_MAX_BYTES, DRIVER_ERROR_CODES, DriverError, } from './types.js';
/**
 ...
 */
export function classifyCommanderTerminal(events) {
 ...
 */
export function extractCommanderInstruction(events, sessionId) {
    const terminal = classifyCommanderTerminal(events);
    if (!terminal.completed || terminal.turn === null) {
        throw new DriverError(DRIVER_ERROR_CODES.COMMANDER_TERMINAL_NOT_COMPLETED, `Commander session '${sessionId}' authoritative terminal is '${String(terminal.reasonKind)}' (turn ${String(terminal.turn)}); only 'completed' is acceptable. No Flash worker is spawned.`);
    }
    ...
    if (finalMessageEvent === null) {
        throw new DriverError(DRIVER_ERROR_CODES.COMMANDER_OUTPUT_MISSING, `Commander session '${sessionId}' completed turn ${turn} has no assistant/message event; no authoritative commander instruction exists`);
    }
    const message = finalMessageEvent.data.message;
    const textBlocks = [];
    ...
    if (textBlocks.length === 0) {
        throw new DriverError(DRIVER_ERROR_CODES.COMMANDER_OUTPUT_EMPTY, `Commander session '${sessionId}' completed turn ${turn} final assistant message has no text blocks`);
    }
    const concatenated = textBlocks.join('');
    const instruction = concatenated.trim();
    if (instruction.length === 0) {
        throw new DriverError(DRIVER_ERROR_CODES.COMMANDER_OUTPUT_EMPTY, `Commander session '${sessionId}' completed turn ${turn} instruction is empty after trimming`);
    }
    const bytes = Buffer.byteLength(instruction, 'utf8');
    if (bytes > COMMANDER_OUTPUT_MAX_BYTES) {
        throw new DriverError(DRIVER_ERROR_CODES.COMMANDER_OUTPUT_OVERSIZED, `Commander session '${sessionId}' instruction is ${bytes} UTF-8 bytes, exceeding the ${COMMANDER_OUTPUT_MAX_BYTES}-byte cap; no Flash worker is spawned and the instruction is NOT silently truncated`);
    }
    return { instruction, bytes, turn };
}
function renderWriteAuthority(rules) {
    if (rules.length === 0) {
        return '  (none)';
    }
    return rules
 ...
 */
export function buildCommanderPrompt(contract, slice, effectiveWorkerToolNames) {
    const lines = [];
 ...
 */
export function buildFlashPrompt(slice, commanderInstruction) {
    const lines = [];
